...
------------------------------------------
local TimeTab2 = Window:MakeTab({
    Name = "นับเวลา 2",
    Icon = "rbxassetid://4483345998",
    PremiumOnly = false
})

local TimeSection2 = TimeTab2:AddSection({
    Name = "Settings"
})

-- เพิ่มตัวแปรใน LocalData สำหรับ TimeTab2
LocalData.TimeWanted2 = LocalData.TimeWanted2 or 0
LocalData.GameTime = LocalData.GameTime or 0
LocalData.IsTimeMode2 = LocalData.IsTimeMode2 or false

TimeSection2:AddTextbox({
    Name = "นับเวลาถอยหลังกี่ชั่วโมง?",
    Default = "0",
    TextDisappear = false,
    Callback = function(Value)
        ...
        if hours > 0 then
            resetModes()
            LocalData.TimeWanted2 = hours * 3600
            LocalData.IsTimeMode2 = true
            LocalData.WebhookSent = false
            WebhookSent = false
            saveSettings(LocalData)
        end
...
TimeSection2:AddButton({
    Name = "Test Webhook",
    Callback = function()
        if LocalData.TimeWanted2 > 0 then
            -- แปลงจำนวนชั่วโมงเป็น วัน:ชั่วโมง
            local totalHours = math.floor(LocalData.TimeWanted2 / 3600)
            local days = math.floor(totalHours / 24)
            local hours = totalHours % 24
            
            local timeStr = string.format(
                "Test!\nPlayer : ||%s||\n- Gem : %d\n- Gold : %d\n- Level : %s\n- Battle Pass : %s [%s]\n\n```md\n#Time Info\n- ตั้งเวลาฟาม : %d วัน : %d ชั่วโมง```",
                player.Name, currentGems, currentGold, currentLevel, currentBP, currentBP2,
                days, hours
            )
            sendWebhook2("<a:alert:1021734820461674527> Test Time2 Notification <a:alert:1021734820461674527>", timeStr)
        end
    end    
})

-- เพิ่มฟังก์ชันสำหรับแปลงเวลาจาก "mm:ss" เป็นวินาที
...
-- ปรับปรุงฟังก์ชัน SendGameFinishedWebhook2
local function SendGameFinishedWebhook2()
    if LocalData.IsTimeMode2 and GameFinished and GameFinished.Value == true then
        local stats = {
            Time = player.PlayerGui.ResultsUI.Holder.Middle.Timer.Text
        }
        
        -- แปลงเวลาเกมเป็นวินาที
        local gameTimeSeconds = timeStringToSeconds(GetTimeFromString(stats.Time))
        LocalData.GameTime = (LocalData.GameTime or 0) + gameTimeSeconds
        
        -- ตรวจสอบว่าครบเวลาที่ตั้งไว้หรือยัง
        if LocalData.GameTime >= LocalData.TimeWanted2 then
            task.spawn(function()
                local totalGameHours = math.floor(LocalData.GameTime / 3600)
                local totalWantedHours = math.floor(LocalData.TimeWanted2 / 3600)
                
                local success = sendWebhookEnd2(
                    "⏰ ครบเวลาเล่นเกมที่ตั้งไว้แล้ว! ⏰",
                    string.format(
                        "🔒・Username : ||**%s**||\n" ..
                        "```md\n#Time Complete!\n" ..
                        "- เวลาที่ตั้งไว้ : %d ชั่วโมง\n" ..
                        "- เวลาที่เล่นไป : %d ชั่วโมง\n" ..
                        "- สถานะ : ✅ ครบกำหนดเวลาแล้ว!\n" ..
                        "```\n",
                        player.Name,
                        totalWantedHours,
                ...
                if success then
                    LocalData.TimeWanted2 = 0
                    LocalData.GameTime = 0
                    LocalData.IsTimeMode2 = false
                    saveSettings(LocalData)
                end
            end)
        end
